/**
 * 🚀 Arma RPC Manager - Singleton Connection Pool
 * 
 ...
 * 
 * This is the foundation of Arma's performance optimization.
 */
 ...
 */
class ArmaRpcManager {
  private static instance: ArmaRpcManager
   ...
   */
  static getInstance(): ArmaRpcManager {
    if (!ArmaRpcManager.instance) {
      ArmaRpcManager.instance = new ArmaRpcManager()
    }
    return ArmaRpcManager.instance
  }
  ...
  getRpc(config: RpcConnectionConfig): SolanaRpc {
    ...
    if (this.rpcPool.size >= this.MAX_CONNECTIONS) {
      console.warn('[Arma RPC] Max connections reached, reusing oldest connection')
    ...
    console.log(`[Arma RPC] Creating new RPC connection: ${key}`)
  ...
  getWebSocket(config: RpcConnectionConfig): SolanaRpcSubscriptions {
    ...
    if (this.wsPool.size >= this.MAX_CONNECTIONS) {
      console.warn('[Arma RPC] Max WebSocket connections reached, reusing oldest')
    ...
    console.log(`[Arma RPC] Creating new WebSocket connection: ${key}`)
   ...
   */
  cleanup(): void {
    console.log('[Arma RPC] Cleaning up all connections')
  ...
  private scheduleCleanup(key: string): void {
    const timer = setTimeout(() => {
      console.log(`[Arma RPC] Cleaning up unused connection: ${key}`)
...
export const rpcManager = ArmaRpcManager.getInstance()
