...
export function parseOklch(color: string): Oklch | null {
  ...
  const num = (s: string, pctScale: number) =>
    s.endsWith('%') ? (Number.parseFloat(s) / 100) * pctScale : Number.parseFloat(s)
  return {
 ...
 */
export function srgbToOklchString(r: number, g: number, b: number, alpha = 1): string {
  const { l, c, h } = srgbToOklch(r, g, b)
  const target: [number, number, number] = [Math.round(r), Math.round(g), Math.round(b)]
...
export function oklchToRgbString(color: string): string {
  ...
  return o.alpha < 1 ? `rgba(${r}, ${g}, ${b}, ${o.alpha})` : `rgb(${r}, ${g}, ${b})`
}

/** Set/replace the alpha of an `oklch()` string — pure string surgery. */
export function withAlpha(color: string, alpha: number): string {
  const m = /^oklch\(\s*([^/)]+?)\s*(?:\/\s*[\d.]+%?)?\s*\)$/i.exec(color.trim())
  if (!m) return color
...
export function adjustOklch(
  ...
  return a >= 1 ? `oklch(${trim(l, 4)} ${trim(c, 4)} ${o.h})` : `oklch(${trim(l, 4)} ${trim(c, 4)} ${o.h} / ${a})`
}
