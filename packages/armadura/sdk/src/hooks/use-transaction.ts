...
export class ArmaTransactionError extends Error {
  constructor(
    message: string,
    public code: string,
    public cause?: Error
    ...
    super(message)
    this.name = 'ArmaTransactionError'
  }
}

export class TransactionSimulationError extends ArmaTransactionError {
  constructor(message: string, cause?: Error) {
    super(message, 'SIMULATION_FAILED', cause)
  }
}

export class TransactionSendError extends ArmaTransactionError {
  constructor(message: string, cause?: Error) {
    super(message, 'SEND_FAILED', cause)
  }
}

export class TransactionConfirmationError extends ArmaTransactionError {
  constructor(message: string, cause?: Error) {
...
export function useTransaction(options: UseTransactionOptions = {}): UseTransactionReturn {
  ...
  const buildTransaction = useCallback(async (
    ...
    try {
      ...
      if (!feePayer) {
        throw new ArmaTransactionError(
      ...
      if (error instanceof ArmaTransactionError) {
        throw error
      }
      if (error instanceof Error) {
        throw new ArmaTransactionError('Failed to build transaction', 'BUILD_FAILED', error)
      }
      throw new ArmaTransactionError('Unknown error building transaction', 'BUILD_FAILED')
    }
    ...
    mutationFn: async ({ instructions, config: txConfig = {} }: SendTransactionParams): Promise<TransactionResult> => {
      ...
      if (!transport) {
        ...
        if (error instanceof Error) {
          ...
          throw new ArmaTransactionError('Transaction failed', 'UNKNOWN_ERROR', error)
        }
        
        throw new ArmaTransactionError('Unknown transaction error', 'UNKNOWN_ERROR')
      }
    },
  ...
  const sendPrebuilt = useCallback(async (tx: { wireTransaction: Uint8Array }, txConfig: TransactionConfig = {}) => {
    try {
      ...
      try {
        ...
        console.warn('[Arma] WebSocket confirmation failed, using polling:', wsError)
        const start = Date.now()
        // eslint-disable-next-line no-constant-condition
        while (true) {
          const { value } = await (rpc as any).getSignatureStatuses([sentSignature]).send()
      ...
      if (error instanceof ArmaTransactionError) throw error
      ...
      if (isUserRejection) {
        throw new ArmaTransactionError('Transaction cancelled by user', 'USER_CANCELLED', error as Error)
      }
      
      if (error instanceof Error) {
        // eslint-disable-next-line no-console
        console.error('[Arma/useTransaction] sendPrebuilt failed:', error)
        throw new ArmaTransactionError(`Failed to send prebuilt transaction: ${error.message}`,'SEND_FAILED', error)
      }
      
      // eslint-disable-next-line no-console
      console.error('[Arma/useTransaction] sendPrebuilt unknown error:', error)
      throw new ArmaTransactionError('Unknown error sending prebuilt transaction', 'SEND_FAILED')
    }
  }, [getRpcClient, wallet.signer])
