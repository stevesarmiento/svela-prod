...
/** Shared grid template for the token/7d/30d-style rows. */
const gridCols = "minmax(0,1.2fr) repeat(2, minmax(0,1fr))";

/** 7d/30d returns per token, with cross-token comparable meters. */
function ReturnsSection({ stats }: { stats: ComparativeStats }) {
  // Shared meter domain so bar lengths compare ACROSS tokens.
...
/** Volatility + beta vs the benchmark token. */
function RiskSection({ stats }: { stats: ComparativeStats }) {
  // Shared meter domain so bar lengths compare ACROSS tokens.
...
/** Pairwise 30d correlation matrix. */
function CorrelationSection({ stats }: { stats: ComparativeStats }) {
...
function IndicatorsSection({ stats }: { stats: ComparativeStats }) {
  return (
...
/** RSI + derivatives-flow readouts and excess-return summary. */
function MomentumFlowSection({ stats }: { stats: ComparativeStats }) {
  return (
    <Section title="Momentum & Flow">
      ...
      <p className="text-[10px] leading-snug text-zinc-500">
        ...
        {stats.tokens
          .flatMap((t) => {
            if (t.id === stats.benchmarkId) return [];
            const pp = (v: number | null) =>
              v === null ? "—" : `${v > 0 ? "+" : ""}${v.toFixed(1)}pp`;
            return [
              `${t.symbol.toUpperCase()} ${pp(t.excessReturn7dPct)} / ${pp(t.excessReturn30dPct)}`,
            ];
          })
          .join(" · ")}
      </p>
...
export function ComparativeStatsPanel({
  ...
  if (!stats) {
    const skeletonKeys = Array.from(
      { length: Math.max(expectedCount, 2) * 3 },
      (_, i) => `stat-skeleton-${i}`,
    );
    return (
      <div className="space-y-3 p-4">
        ...
        {skeletonKeys.map((key) => (
          <Skeleton key={key} className="h-3 w-full rounded-full" />
        ))}
      </div>
    );
  }
  ...
  return (
    <ScrollArea hideScrollbar className="h-[75vh]">
      <div className="space-y-3 py-4">
        ...
        <Divider />
        <MomentumFlowSection stats={stats} />
      </div>
    </ScrollArea>
  );
}
