...
function marketsUrl(args: {
  ids?: ReadonlyArray<string>;
  perPage: number;
  sparkline: boolean;
}): string {
  const url = new URL(`${COINGECKO_PRO_BASE_URL}/coins/markets`);
...
function coinUrl(id: string): string {
  const url = new URL(`${COINGECKO_PRO_BASE_URL}/coins/${encodeURIComponent(id)}`);
...
function quoteFromMarketsRow(row: CoinGeckoMarketsRow) {
  const sparkline7d =
  ...
  (request: NextRequest, _ctx, session) =>
    Effect.gen(function* () {
      ...
      if (ids.length > 0) {
        ...
        const symbolMatches = yield* Effect.all(
          symbols.map((symbol) =>
            convex.serverQuery(
              api.coins.getCoinGeckoCoinsBySymbol,
              { symbol },
              { label: "getCoinGeckoCoinsBySymbol" },
            ),
          ),
          { concurrency: 16 },
        );
        ...
        const nameMatches = yield* Effect.all(
          names.map((name) =>
            convex.serverQuery(
              api.coins.searchCoinGeckoCoins,
              { query: name, limit: Math.min(25, limit) },
      ...
      if (apiKey) {
        ...
        const fetchMarkets = (chunkIds?: ReadonlyArray<string>, perPage?: number) =>
          http.requestJson({
            vendor: "coingecko",
            endpoint: marketsUrl({
              ids: chunkIds,
              perPage: perPage ?? chunkIds?.length ?? limit,
              sparkline,
            }),
            decode: decodeMarketsRows,
            init: { headers, next: { revalidate: 30 } },
            timeoutMs: 15_000,
          });
        ...
        const rows =
          resolvedIds.length > 0
            ? (yield* Effect.all(
                chunk(resolvedIds, 250).map((idChunk) => fetchMarkets(idChunk)),
                { concurrency: 3 },
              )).flat()
            : yield* fetchMarkets(undefined, limit);

        for (const row of rows) {
          data[row.id] = quoteFromMarketsRow(row);
        }
        ...
        if (resolvedIds.length > 0) {
          ...
          if (missingIds.length > 0) {
                ...
                missingIds.map((id) =>
                  http
                    ...
                    .pipe(
                      ...
                      // Per-item swallow (matches previous behavior), but visible in logs.
                      Effect.catch((error) =>
                        Effect.sync(() => {
                          console.warn(`[coingecko-quotes] per-coin fallback failed for ${id}: ${error._tag}`);
                        }),
                      ),
                    ),
                ),
            ...
            if (stillMissing.length > 0) {
              yield* convex.warmup(
                api.coingeckoWarmup.requestMarketsRefresh,
                { coingeckoIds: stillMissing },
                "coingecko-quotes:requestMarketsRefresh",
              );
            }
          }
          ...
          for (const itemChunk of chunk(marketItems, 100)) {
            yield* convex.warmup(
              api.coingeckoMarkets.upsertMarketDataBatch,
              { items: itemChunk },
              "coingecko-quotes:upsertMarketDataBatch",
            );
          }
        }
      } else {
        ...
        const marketDocs =
          resolvedIds.length > 0
            ? yield* Effect.all(
                resolvedIds.map((id) =>
                  convex.serverQuery(
                    api.coingeckoMarkets.getMarketDataByCoingeckoId,
                    { coingeckoId: id },
                    { label: "getMarketDataByCoingeckoId" },
                  ),
                ),
                { concurrency: 16 },
              )
            : yield* convex.serverQuery(
