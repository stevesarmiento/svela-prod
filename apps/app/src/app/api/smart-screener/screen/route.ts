...
import {
  SMART_SCREENER_METRICS,
  getSmartScreenerMetric,
} from "@/lib/smart-screener/metric-registry";
...
function scoreRowAgainstMarketFilters(args: {
  ...
  for (const f of args.filters) {
    ...
    const left = rawValue;
    const right = normalizeFilterValue({
...
function scoreRowAgainstTechnicalFilters(args: {
  ...
  seriesByDays: Record<
    string,
    ReadonlyArray<PriceHistoryPointLike> | undefined
  >;
  missingByMetricId: Record<string, number>;
}): boolean {
  for (const f of args.filters) {
...
function sortRows(args: {
  ...
  seriesById: Record<
    string,
    Record<string, ReadonlyArray<PriceHistoryPointLike> | undefined> | undefined
  >;
...
- If you are not confident, set confidence <= 0.4 and return an empty filters list.
  ...
  const restrictIds =
    Array.isArray(current?.coingeckoIds) && current.coingeckoIds.length > 0
      ? new Set(current.coingeckoIds)
      : null;
  ...
  const prefilterCap = needsTechnical
    ? Math.min(600, Math.max(150, dsl.limit * 4))
    : dsl.limit;

  for (let page = 0; page < maxPages; page += 1) {
    const pageRows = await convex.query(
      api.coingeckoMarkets.getMarketDataPageByRank,
      {
  ...
  if (needsTechnical && prefilteredRows.length > 0) {
    ...
    const uniqueIds = Array.from(
      new Set(prefilteredRows.map((r) => r.coingeckoId)),
    ...
    async function loadSeries(inner: {
      ...
      points: ReadonlyArray<PriceHistoryPointLike>;
      stale: boolean;
    }> {
    ...
    for (const row of prefilteredRows) {
      ...
      if (
        !scoreRowAgainstTechnicalFilters({
          row,
          filters: dsl.filters,
          seriesByDays: byDays,
          missingByMetricId,
  ...
  const sorted = sortRows({ rows: matchedRows, dsl, seriesById }).slice(
    0,
    dsl.limit,
  );
  ...
  return final.success
    ? final.data
    : (resp as z.infer<typeof SmartScreenerScreenResponseSchema>);
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    const rateLimitResult = await ratelimit.limit(
      `${ip}-${clerkId}-smart-screener-screen`,
    );
  ...
  // Fire-and-forget usage tracking for the admin dashboard.
  convex
    .mutation(api.aiUsage.recordAiFeatureUsage, {
      serverToken: getServerToken(),
      clerkId,
      feature: "screener_search",
    })
    .catch(() => {});
  ...
  return NextResponse.json(final.success ? final.data : result, {
    status: 200,
  });
}
