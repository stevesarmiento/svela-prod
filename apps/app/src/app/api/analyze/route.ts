...
export const POST = withAuthRatelimit(
  async (req: NextRequest) => {
    ...
    auth()
      .then(({ userId }) => {
        if (!userId) return;
        return convex.mutation(api.aiUsage.recordAiFeatureUsage, {
...
async function handleAnalyze(req: Request) {
  try {
    ...
    console.log(
      "Support/Resistance:",
      validatedData.priceContext?.support,
      "/",
      validatedData.priceContext?.resistance,
    );
    console.log(
      "REAL Open Interest:",
      validatedData.liquidationData?.openInterest,
    );
    console.log(
      "REAL Liquidations 24h:",
      validatedData.liquidationData?.totalLiquidations24h,
    );
    console.log(
      "REAL Buy/Sell:",
      validatedData.orderFlow?.takerBuyRatio,
      "ratio",
    );
    console.log("REAL Buy Volume:", validatedData.orderFlow?.buyVolumeUsd);
    console.log(
      "Volume Analysis:",
      validatedData.volumeAnalysis?.volumeTrend,
      validatedData.volumeAnalysis?.volumeSpike,
    );
    console.log(
      "Price History Length:",
      validatedData.priceContext?.priceHistory?.length,
    );
    console.log(
      "RSI History Length:",
      validatedData.marketVision?.rsi?.history?.length,
    );
    console.log("===========================================");
    ...
    const longLiquidations =
      validatedData.liquidationData?.longLiquidations ?? 0;
    const shortLiquidations =
      validatedData.liquidationData?.shortLiquidations ?? 0;
    const directionalLiquidationsTotal = longLiquidations + shortLiquidations;
    const longLiquidationsPct =
      directionalLiquidationsTotal > 0
        ? (longLiquidations / directionalLiquidationsTotal) * 100
        : 0;
    const shortLiquidationsPct =
      directionalLiquidationsTotal > 0
        ? (shortLiquidations / directionalLiquidationsTotal) * 100
        : 0;
