...
/** Fallback prose when the generated brief is missing, stale, or too long. */
function buildBriefSummary(args: {
  brief: DailyBrief | null
  movers: SnapshotMovers | null | undefined
  events: SnapshotEvent[]
}): string {
  const { brief, movers, events } = args
  const now = Date.now()
  const windowEvents = filterEventsInWindow(events, "24h", now)
...
/** Locally derived regime/technicals/theme cards when the brief lacks them. */
function buildFallbackCards(args: {
  movers: SnapshotMovers | null | undefined
  events: SnapshotEvent[]
}): BriefCard[] {
  const { movers, events } = args
  const now = Date.now()
  const windowEvents = filterEventsInWindow(events, "24h", now)
...
/** Shared fixed-height slide shell with the enter/exit fade. */
  ...
  return (
    <div className={cn(SLIDE_MIN_H, "flex flex-col justify-center px-2 py-4 pt-0")}>
      <AnimatePresence mode="wait">
        <MotionDiv
          key={props.slideKey}
          initial={props.shouldReduceMotion ? false : { opacity: 0, y: 0 }}
          animate={{ opacity: 1, y: 0 }}
          exit={props.shouldReduceMotion ? undefined : { opacity: 0, y: 0 }}
          transition={{ duration: props.dur, ease: EASE_OUT_CUBIC }}
...
/** One brief card slide: label + tone badge, rich body, per-kind detail block. */
function BriefCardSlideContent(props: { card: BriefCard; tokens: Map<string, TokenRef> }) {
  const { card, tokens } = props
  return (
    <>
      ...
      <div className="text-base leading-relaxed text-zinc-700 dark:text-zinc-200 text-pretty">
        {renderRichSummary(card.body, tokens)}
      </div>
...
/** Top movers slide content: 24h badge plus the two mover columns. */
function MoversSlideContent(props: { gainers: SnapshotMover[]; losers: SnapshotMover[] }) {
  return (
    <>
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-xs uppercase tracking-wide text-muted-foreground">
      ...
      <div className="grid grid-cols-2 gap-4 pt-1">
        <MoversList title="Top gainers" rows={props.gainers} />
        <MoversList title="Top losers" rows={props.losers} />
      </div>
    </>
  )
}
...
function useSlideAutoplay(
  ...
  return { isPaused, setIsPaused, handleDotClick }
}

/** Autoplay progress dots under the carousel. */
function SlideProgressDots(props: {
  slideKeys: string[]
  selectedSlide: number
  isPaused: boolean
  ...
  return (
    <div className="flex items-center justify-start gap-1.5 px-2">
      ...
      {props.slideKeys.map((slideKey, idx) => {
        const isActive = idx === props.selectedSlide
...
export function OverviewDailyBriefCard(props: {
  ...
  // freshest — a newer cached brief supersedes an older locally generated one.
  const brief = useMemo(() => {
    const cached = cache?.brief ?? null
    if (!generatedBrief) return cached
    if (cached?.generatedAt && cached.generatedAt >= generatedBrief.generatedAt) return cached
  ...
  const { isPaused, setIsPaused, handleDotClick } = useSlideAutoplay(
    carouselApi,
    selectedSlide,
    slideKeys.length,
  )
        ...
        >
          ...
          >
            <CarouselContent>
              <CarouselItem>
                <AnimatedSlide
                  slideKey={`summary-${selectedSlide}`}
              ...
              {orderedCards.map((card) => (
                <CarouselItem key={card.kind}>
                  <AnimatedSlide
                    slideKey={`card-${card.kind}-${selectedSlide}`}
                    shouldReduceMotion={shouldReduceMotion}
                    dur={dur}
                    className={SLIDE_CARD_CLASS}
                  >
                    <BriefCardSlideContent card={card} tokens={tokenIndex} />
                  </AnimatedSlide>
                </CarouselItem>
              ))}
              ...
              {moversSlide ? (
                <CarouselItem key="movers">
                  <AnimatedSlide
                    slideKey={`card-movers-${selectedSlide}`}
                    shouldReduceMotion={shouldReduceMotion}
                    dur={dur}
                    className={SLIDE_CARD_CLASS}
                  >
                    <MoversSlideContent
                      gainers={moversSlide.gainers}
                      losers={moversSlide.losers}
                    />
                  </AnimatedSlide>
                </CarouselItem>
              ) : null}
