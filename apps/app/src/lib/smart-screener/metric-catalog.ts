...
export type MetricSource = "markets" | "taker";
...
export interface CoingeckoMarketRowLike {
  ...
  // Precomputed technicals (written by the 4h Convex top-markets cron;
  // see convex/_lib/technicalMetrics.ts for the shared definitions).
  return7dPct?: number;
  return30dPct?: number;
...
export interface MarketsMetricDefinition {
  ...
  description?: string;
  source: "markets";
  /** Signed metrics (e.g. USD deltas) opt out of the usd >= 0 validation. */
...
function pctChangeFromCurrentAndReference(args: {
  ...
  if (!Number.isFinite(args.current) || !Number.isFinite(args.reference))
    return null;
...
function pctRangeFromHighLowAndCurrent(args: {
  ...
  if (args.high == null || args.low == null || args.current == null)
    return null;
  if (
    !Number.isFinite(args.high) ||
    !Number.isFinite(args.low) ||
    !Number.isFinite(args.current)
  )
    return null;
...
export const SMART_SCREENER_MARKET_METRICS: ReadonlyArray<MarketsMetricDefinition> =
  [
    ...
    },
    {
      ...
      source: "markets",
      getValue: (r) =>
        typeof r.totalVolume === "number" ? r.totalVolume : null,
    },
    {
      id: "fdv_usd",
      label: "FDV",
      ...
      source: "markets",
      getValue: (r) =>
        typeof r.fullyDilutedValuation === "number"
          ? r.fullyDilutedValuation
          : null,
    },
    {
      id: "market_cap_rank",
      ...
      source: "markets",
      getValue: (r) =>
        typeof r.marketCapRank === "number" ? r.marketCapRank : null,
    },
    {
      ...
      source: "markets",
      getValue: (r) =>
        typeof r.priceChangePercentage24h === "number"
    ...
    },
    {
      ...
      source: "markets",
      getValue: (r) =>
        typeof r.marketCapChangePercentage24h === "number"
    ...
    },
    {
      ...
      allowNegative: true,
      getValue: (r) =>
        typeof r.priceChange24h === "number" ? r.priceChange24h : null,
    },
    {
      ...
      unit: "usd",
      synonyms: [
        "market cap change 24h usd",
        "mcap delta 24h",
        "market cap delta 24h",
      ],
      source: "markets",
      allowNegative: true,
      getValue: (r) =>
        typeof r.marketCapChange24h === "number" ? r.marketCapChange24h : null,
    },
    {
      id: "range_24h_pct",
      label: "24h range (%)",
    ...
    },
    {
      ...
      description:
        "(ath - currentPrice) / ath * 100 (positive means below ATH)",
      source: "markets",
      getValue: (r) =>
