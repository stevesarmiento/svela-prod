...
export function useHoldingsValueOverTime(args: {
  ...
  const stableIdsKey = positions
    .map((row) => row.coinId.trim())
  ...
  const query = useQuery({
    queryKey: ["holdings-value-over-time", stableIdsKey, args.timeScale, rangeEndTimeMs],
    queryFn: async (): Promise<LivelinePoint[]> => {
      ...
      const series = results.filter(
        (row): row is { coinId: string; holdings: number; prices: Array<{ time: number; value: number }> } =>
      ...
      const totalsByBucket: Array<number> = Array.from({ length: bucketTimesMs.length }, () => 0)

      for (const coin of series) {
        const sorted = [...coin.prices].sort((a, b) => a.time - b.time)
        const baselinePrice = sorted[0]?.value && sorted[0].value > 0 ? sorted[0].value : null
        ...
        for (let i = 0; i < bucketTimesMs.length; i++) {
          ...
          while (cursor < sorted.length && sorted[cursor]!.time <= bucketTimeSec) {
            lastPrice = sorted[cursor]!.value
            cursor++
          }

          const priceForBucket = lastPrice ?? baselinePrice
          totalsByBucket[i] = (totalsByBucket[i] ?? 0) + coin.holdings * priceForBucket
        }
      }
