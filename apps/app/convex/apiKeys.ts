...
export const getMyApiKeyStats = query({
  args: {},
  returns: v.object({
    totalKeys: v.number(),
    activeKeys: v.number(),
...
export const updateMyApiKeyStatus = mutation({
  args: { keyId: v.id("userApiKeys"), isActive: v.boolean() },
  returns: v.null(),
  handler: async (ctx, args) => {
    const user = await getCurrentUser(ctx);
    if (!user) throw new Error("Not authenticated");
    ...
    const apiKey = await ctx.db.get(args.keyId);
    if (!apiKey || apiKey.userId !== user._id) throw new Error("API key not found or access denied");

    await ctx.db.patch(args.keyId, { isActive: args.isActive, updatedAt: Date.now() });
    return null;
  },
});
...
export const _upsertMyApiKeyEncrypted = internalMutation({
  args: {
    provider: v.string(),
    keyName: v.string(),
    encryptedKey: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    const providerConfig = API_PROVIDERS[args.provider as keyof typeof API_PROVIDERS];
    if (!providerConfig) throw new Error(`Invalid API provider: ${args.provider}`);
